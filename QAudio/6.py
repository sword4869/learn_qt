    ...
    def initialize_audio(self, audio_device: QAudioDevice):
        ...
        def push_mode_slot():
            ...
            if length > 0:
                ...
                # 逻辑链：
                #       音量大于0.6
                #           进入识别，setText('检测唤醒词中...')，开启线程，线程期间保持不变
                #       音量小于等于0.6
                #           setText('听不见')
                #####
                if self.parent.wakeup_thread.isRunning() == False:
                    if level > 0.6:
                        self.label.setText('识别唤醒词中...')
                        self.parent.wakeup_thread.start()
                    else:
                        self.label.setText('听不见')
